...
- **未検証の申し送り**: `makensis` は POSIX ホストでも動くのでスクリプトのコンパイルと同梱物の中身は検証済みだが、**Windows 実機でのインストール動作は未確認**。上書きアップグレード、関連付けの登録/解除、昇格を落とした起動の 3 点は実機で確認してほしい（`build_installer.ps1` が最後に出す smoke checklist がそのまま手順になる）。
