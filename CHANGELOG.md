...
- **フォーカス中に Space を押すと音量が飛んでいた**。egui はフォーカスされたクリック可能
  ウィジェットで Space / Enter をクリックに変換する（`FAKE_PRIMARY_CLICKED`）ため、
  フェーダーがそれをクリックとして読み、**マウスが置きっぱなしの位置**に音量を書いていた。
  しかも Space は消費されないので再生トグルも同時に走る。位置の取得元を
  `Response::interact_pointer_pos`（このウィジェットをポインタが実際に操作している
  ときだけ `Some`）に変え、キーボード由来の合成クリックでは何も起きないようにした。
- **左右キーが二重に効いていた**。フェーダーは focus 中に Left/Right を消費するが、
  Editor のシークは `key_down`（保持キー集合。`consume_key` は `events` から取り除く
  だけで、こちらには触らない）を読み、リストは「他のウィジェットが消費した押下」の
  ための保険として raw イベントログを数える。どちらも消費では止まらない。両方に
  `topbar_volume_owns_arrows` を尋ねさせ、トップバーが先に描かれる順序に依存しない形にした。
- **ダブルクリックで 0 / -6 / -18 / -24 dB を送る**ようにした（従来は 0 dB 固定リセット）。
  ループの起点は「1 回目のクリックが見つけた値」に固定してある。1 回目は位置指定分岐を
  先に通って音量を書き換えてしまうので、現在値を読むと毎回 unity に戻ってしまうため。

### Tab / Shift+Tab の循環に List を入れた

- 従来は Editor タブが 2 つ以上あるときだけ、Editor タブ間だけを回っていた。
  `Ctrl+1 = List` / `Ctrl+2.. = Editor タブ` と同じ並びで **List をスロット 0** に入れ、
  タブストリップの見た目どおりに一周するようにした。Editor タブが 0 個のときと
  Recording / Effect Graph が前面のときは、従来どおり egui のフォーカス移動のまま。
- **フォーカスの後始末を切り替えより先に**行うようにした。List へ移る経路はフォーカスを
  取りに行くので、順序が逆だと取った直後に手放してしまう。
- List へ切り替える本体（プレビュー破棄・`pending_*` クリア・フォーカス）は 3 箇所に
  同じものが散っていたので `activate_list_workspace` に集約した。**別ワークスペースから
  来た場合は意図を記録するだけ**にしてある — egui は `Memory::focused` を AccessKit の
  更新にそのまま書き込み、その ID が収集済みノードに含まれるか検証しないので、
  そのパスで描かれていないウィジェットにフォーカスを要求すると壊れたツリーになる。

### コメントのボタンが豆腐（□）になっていたのを直した

- コメントポップアップ右上の四角は **U+29C9 ⧉ TWO JOINED SQUARES**。この符号位置は
  **アプリが積むフォントのどれにも無い** — バンドルの NotoSansJP にも、epaint 既定の
  4 フォントにも、`theme_ops` が探すシステムフォント候補にも。フォント順序の問題では
  ない（同じ画面の `⟳` `🔗` `💬` は絵文字フォールバックで正常に出ている）。
- すぐ上の `comment_cancel_button` が ✕ を線分で描いているのと同じ理屈で、**あの字が
  表している「ずれた 2 つの四角」を直接描く**ようにした。手前の四角はボタン面で塗って
  から枠線を引いている（枠線 2 枚だけだと交差して格子に見え、奥行きが出ない）。
  ウィンドウヘッダの dock トグルも同じ字だったので一緒に直した。
- **同種の潜在バグも 2 箇所**: `✕` U+2715 はシステムフォントにしか無く、非同期の
  フォント昇格が載るまで、また候補フォントが無い環境では永久に豆腐。Licenses と
  Recording の該当箇所を、全フォントが持つ `×` U+00D7 に置き換えた。隣の `✔` は
  NotoEmoji が持っているのでそのまま。

### Editor のキャンバス右クリックから、Comment に参照を張れるようにした

- 右クリックで **Selection / Playhead / File** を選ぶと、Comments ウィンドウが開いて
  入力欄に参照トークン（`@[path|start-end|low-highHz]`）が入った状態になる。
  スペクトログラム上でドラッグした選択は**周波数帯も一緒に**乗る（`freq_selection` は
  時間範囲と同じドラッグで入る値なので、「スペクトル範囲」という独立項目は作っていない）。
- **既存の右ボタン操作は取り上げない**。egui のコンテキストメニューは `secondary_clicked`
  ＝ドラッグではなくクリックで開くので、右ドラッグのシークと Shift+右ドラッグの範囲選択は
  そのまま。右クリックである 2 つのジェスチャ（WORLD F0 ペンシルの消しゴム、Spectral Warp
  のポイント削除）は、シークブロックが使っているのと同じ 2 つのフラグで締め出している。
- メニュー項目は Comments 側の `🔗 Reference` と**同じ関数を共有**させた。ラベルと活性
  条件が二重管理になるのを避けるため。Editor はタブを借用したまま呼ぶのでメソッドでは
  なく自由関数にしてある。

### ミニメーターの STEREO パネル右上で文字が重なっていたのを直した

- チャンネルバッジ（`MONO` / `CH1+2`）と数値の相関が**同じ右アンカー**に描かれていて、
  モノラルでは固定値 `+1.00` の上に `MONO` が重なって潰れていた。
- タイトル行を**実測したガレーから組む**ようにして、入らないものは右から落とす。
  ラベルのフォントは名目上 monospace だが、CJK フォントがそのファミリの先頭に挿さり、
  さらにシステムフォント昇格で実行中に差し替わるので、文字数×固定幅の見積もりは
  最初から、そして時間が経つとまた別の意味で外れる。
- 相関値はモノラルでは固定値で情報が無いため、描画そのものをやめた。
