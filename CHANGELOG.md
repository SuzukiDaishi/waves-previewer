...
### Volume を、実際に使う範囲が広いフェーダーにした

- トラックが **dB に対して等間隔**だったので、**0 dB 〜 -24 dB がトラックの 28% しか
  無く**、残り 7 割は「ほぼ無音〜無音」に使われていた。少し動かすだけで大きく変わり、
  モニター音量の微調整がしづらい。
- **0 dB 〜 -24 dB に約 45% を割り当て**、そこから下（-80 dB まで）を圧縮する
  テーパーにした（コンソールのフェーダーと同じ考え方）。**無段階なのは従来どおり**で、
  位置と dB は 1 対 1（連続かつ単調）。往復変換のテストで担保している。
- ユニティ（0 dB）は等間隔フェーダーのような「自然な位置」ではなくなるので、
  **0 dB に目盛り**を入れた。読み出しは（幅があるときは）小数第 1 位まで出す —
  細かく動かせるようにしたのに表示が丸めてしまうのでは意味が無いため。

### Editor の読み込み中の波形が、前から伸びていくのをやめた

- 読み込み中の波形（`loading_waveform_minmax`）を**デコード済みの先頭部分だけから
  作って、それを全ビンに引き伸ばして**いた。キャンバス自体は最初から全長
  （`samples_len_visual`）なので、**2% しかデコードできていない時点で画面の 96% に
  波形が描かれ**、進むたびに違う倍率で描き直される。これが「前からビヨーンと伸びて
  いく」正体で、**同時に再生位置（プレイヘッドは常に全長のタイムライン上にある）と
  描画がずれる**原因でもあった。
- **先に全長ぶんのビンを確保し、届いた部分だけを本来の位置に埋める**ようにした
  （`build_loading_overview_over_total`）。先頭は最初から正しい位置に出て、以後
  動かない。まだ来ていない範囲は平らなまま＝確保済みの空間として見える。
- **全長は「キャンバスの長さ」から導出**する。キャンバスは出力フレーム、届く
  サンプルはソースフレームなので、別々に見積もると両者がずれる。片方から変換する
  ことで、描画とタイムラインがサンプル単位で一致する。
- 長さがまだ分からない経路（ヘッダに尺が無く、変換に使うソースレートも未取得）は、
  **最初のチャンクでレートが分かった時点で見積もりから全長を決める**ようにした。
  それも無い場合だけ従来どおり（＝伸びる）で、その時はキャンバスも同じだけ伸びるので
  ずれは起きない。

### アプリが「タスクマネージャでしか終われない状態」に入れなくした

全体の安定化として、UI スレッドが二度と戻ってこなくなる経路を潰した。

- **【重大】ワーカーが黙って死ぬと、アプリが操作不能のまま固まっていた。** 保存 /
  エクスポート / クリップボード準備 / プレビュー処理の 4 つは、**ワーカーからの
  メッセージだけ**で状態が解除される作りで、受信側は `Err(_) => None`、つまり
  「まだ来ていない」と「もう二度と来ない」を同じ扱いにしていた。ワーカーが panic
  するか結果を送らずに終了すると送信側が落ち、`try_recv` は**以後ずっと**
  `Disconnected` を返す。結果として、
  - 全入力を食う**モーダルオーバーレイが永久に残る**（キャンセルも無い）、
  - 終了ダイアログはそのオーバーレイの**下**に描かれるのでクリックできず、
    **アプリを正常終了できない**（＝未保存の編集ごとプロセス kill しかない）、
  - 再描画も 50ms / 60fps に固定されたままで、何もしていないのに 1 コア回り続ける。
  - `loading_ops::poll_job` / `JobPoll` を入れて、**「まだ」と「もう来ない」を型で
    区別**した。`Gone` は状態を解除し、何が起きたかをトーストで出す（保存なら
    「セッションは書かれていない」と明示する。ディスク上のファイルは無傷なので、
    黙って成功したふりをしないことが要点）。エクスポートは `saving_*` の帳簿も
    一緒に片付ける — 残すと**次のエクスポートの結果に当たってしまう**。
- **生きているが応答しないジョブにも出口を用意した。** 死んだワーカーは 1 フレームで
  検出できるが、共有ドライブが無応答になった場合はワーカーは生きたまま止まる。
  30 秒を超えたらオーバーレイに **「Stop waiting」** を出す。ジョブは止めない
  （止められない）— **ウィンドウを返すだけ**だと明示する。次のジョブはまた
  モーダルになる（一度の詰まりが以後の保存を非モーダルにしてしまわないため）。
- **終了ダイアログはオーバーレイより優先する。** 終了要求が出ている間、
  オーバーレイは描かない。どんなジョブより「閉じたい」が優先で、**答えの返らない
  ジョブが「アプリを終われない理由」になってはいけない**。
- **CSV エクスポートが 1 行のために永久に回り続けることがあった。** 測れなかった行は
  キューに戻す作りで、**戻す回数に上限が無かった**。metadata が永遠に揃わない行
  （ワーカーが死んだ、書き出し中にファイルが消えた）が 1 つあると、全入力を
  ブロックしたまま無限に再試行する。4 回で諦めてその行を空欄で書き、**何行
  測れなかったかをトーストで出す**（黙って短い測定結果を出す方が悪い）。

### 波形クリックのシークが、たまに無かったことになる不具合を直した

- 行の波形をクリックすると、**デコードがそこまで届くまでシークを「駐車」**して待つ
  作りになっている。その駐車を retire する処理が、「**別の行がトランスポートを
  奪った**（＝古いシークなので捨てる）」と「**まだこの行にトランスポートが
  渡っていない**（＝バッファ待ち。待つのが仕事）」を同じ扱いにしていた。
  前者だけを捨てるようにした。
  - 症状は「50% をクリックしたのに、Space を押すと頭から鳴る」。プレビューが
    まだ載っていない行（大きいファイル、共有ドライブ、マシンが忙しい）で出る。
  - 待ちが無限にならないように、**デコードが走っていないときは従来どおり
    「今あるところ」に着地して終わる**経路に落とす。
- 既存の回帰テスト（`list_wave_seek_does_not_start_playback_when_auto_play_is_off`）
  が並列実行時にこれを踏んで落ちていた。修正で発生頻度は下がったが**まだ完全には
  消えていない** — 残りは「シーク適用後に遅れて届いたバッファが位置を 0 に戻す」
  別経路。`docs/UI_FREEZE_COUNTERMEASURES_20260819.md` の第3ラウンドに記録した。

### 列の並べ替えテストが古いフィールドを触っていたのを直した

- `p7_pipeline::column_reorder_moves_headers_and_persists_in_session` は
  `list_column_order` に書き込んで並びが変わることを期待していたが、この
  フィールドは **`list_column_layout` から毎フレーム導出される読み取り専用の
  投影**になっている（メタデータ列を含められるようにした時の変更）。書いても
  何も動かず、次のフレームで上書きされる。**このテストはそれ以来ずっと赤**だった。
- 実際に UI が読む `list_column_layout` を、List Columns ウィンドウと同じ手順
  （レイアウト移動 → revision 更新 → sanitize）で動かすように直した。
  セッション往復も layout で検証するようにした（order はそこから従う）。
- 恒常的に赤いテストは「赤を無視する習慣」を作るので、安定化の一部として直した。

### メニューを開いただけで固まる経路を潰した

- **メニューの中身は開いている間ずっと毎フレーム実行される**。そこに選択範囲ぶんの
  仕事が入っていた。List メニューは `selected_paths()`（= 全選択パスの clone +
  ソート）を**毎フレーム 4 回**、行の右クリックメニューは clone 2 回に加えて
  **タグ定義数 × 選択行数**の照会を毎フレーム。10 万行を Ctrl+A した状態で
  右クリックすると、それだけでアプリが止まる。
  - 「何行あるか」「2 行以上か」は上限付きカウント（`selection_has_at_least`）に、
    ステータス共有 / タグ状態 / 変換可否 / 未保存編集の有無は **1 回のパスに
    まとめて 250ms キャッシュ**（`SelectionMenuSummary`）にした。メニューが開いて
    いる間に選択は変わらないので、キャッシュしても答えは同じ。パスの実体は
    **クリックされた項目の中でだけ**作る。
- **Paste の有効判定が、毎フレーム OS のクリップボードを 2 回開いていた。**
  Windows ではクリップボードは**他のアプリも取り合うグローバルなロック**で、
  秒 120 回開けば他アプリのコピペを失敗させうるし、巨大なテキストが載っていれば
  毎フレーム丸ごとコピーすることになる。250ms キャッシュにした。
- **Command Palette は毎フレーム設定ファイルを stat していた**（しかもパス解決が
  `create_dir_all` を呼ぶので、ディレクトリ作成 API を毎フレーム叩いていた）。
  1 秒キャッシュ + サンプル生成時に破棄、に変えた。

### リストの Comments 列から、その行のコメントを読み書きできるようにした

- **リストに Comments 列を足した**（既定で表示）。セルは `💬 N` で、**N はその
  ファイルを指すスレッド全体の件数**。未読があれば青丸、根が全部 resolve 済みなら
  弱色で出す。コメントが無い行は、ホバーしたときだけ薄い `💬+` を出す — 10 万行の
  リストに 10 万個の吹き出しを描いて「何も無い」と言う意味は無い。
- **セルを押すとその行の会話がポップアップで開く**。描画は**ウィンドウとまったく
  同じスレッド描画器**なので、Reply / Edit / Delete / Resolve がそのまま効く。
  2 つ目の簡易描画器を作れば 1 リリースでウィンドウとずれる。
- **ポップアップから書いたコメントには、そのファイルの参照トークンが自動で付く**。
  仕様上「本文中のトークンが唯一の真実」なので、行から書いたのにどのファイルの
  話か記録されていない、という状態を作らない。返信には付けない（スレッドの根が
  既に言っているので、二重に書くだけになる）。
- **開いた時点で、その行のスレッドは既読になる**。ウィンドウを開くのと同じ意味。
- `⧉` でウィンドウをその行に向けて開く。列を隠していても、行の右クリックメニューの
  `Comments...` から同じ場所へ行ける。ヘッダの `💬` は件数ソート（降順が先なので、
  1 クリックで「何か言われている行」が上に来る）。
- **件数は行ごとに数えない**。会話が動いたときだけスレッドを 1 周して
  `パス → (件数, 未解決, 未読)` を作り、セルはハッシュ 1 回で読む。行ごとに本文の
  参照トークンを走査すると「行数 × コメント数」を毎フレーム払うことになる。

### 上下キーでリストを動かすと、たまに行で引っかかって進まなくなる不具合を直した

原因は 2 つあり、どちらも「押したのに効かない」に見える。

- **フォーカスが列の外へ歩いていっていた（本命）。** egui は、フォーカス中の
  ウィジェットが矢印キーを要求していないとき、矢印を**フォーカス移動**として解釈する。
  その要求（`Memory::set_focus_lock_filter`）は「**前のフレームでもフォーカスを
  持っていたウィジェット**」にしか結び付かないので、`request_focus` を新しく呼ぶと
  フィルタは既定値に戻り、**次の 1 フレームだけ**リストの矢印がフォーカス移動になる。
  リストは矢印を受けるたびに `request_focus` を呼んでいたので、この隙間が毎回開いていた。
  行き先は検索ボックスやトップバーの DragValue — どちらもテキスト入力なので、
  そこにキャレットが立つとリストのキーは**全部**止まり、クリックするまで戻らない。
  これが「その行で引っかかって進まなくなる」の正体。
  - **すでに持っているフォーカスは取り直さない**ようにした（取り直しがフィルタを
    消していた）。取るときは 1 フレーム余分に要求して、次のキーが来る前に
    フィルタが張られている状態にする。
  - 保険として、**矢印を処理した次のフレームにフォーカスが外へ出ていたら取り返す**。
    ポインタ押下も、ユーザーが打った修飾キー付きの操作（Ctrl+F は検索ボックス）も
    無かったときだけ。矢印にフォーカス移動をさせないのが本筋で、こちらは網。
- **1 フレームに 1 行しか進まなかった。** キーリピートはフレームより速いことがあり、
  ソートやデコードで 1 フレームが伸びた瞬間に、その間のリピートは**まとめて 1 回**に
  潰れていた。押しっぱなしにすると、重い行に来たところで進みが止まって見える。
  **そのフレームに届いた押下数を数えて、その分だけ動かす**ようにした（PageUp/Down も
  同じ）。読み込むのは実際に着地した行だけなので、通り過ぎた行のプレビューは走らない。
- ついでに、選択インデックスが**リストの範囲外に残っていると矢印が完全に無反応**に
  なる経路も塞いだ（`update_selection_on_click` も `select_and_load` も範囲外の行を
  拒否するので、範囲外から 1 歩動いても範囲外のまま）。歩く前に末尾へ丸める。

### 行ごとのステータスとタグを追加した

- **リストに Status 列（1 行 1 つ）と Tags 列（1 行に複数）を足した**。これまで行に書けたのは自由記述の Note だけで、「WIP / レビュー待ち / OK」のような**決まった選択肢での仕分け**ができなかった。セルはプルダウン（Tags はチェック式）で、色付きバッジで表示する。Status は既定で表示、Tags は既定で非表示（Tools → List Columns… で切り替え）。
- **ラベルの管理は Tools → Statuses & Tags…**。追加・リネーム・色・並び順・削除と、デフォルトステータスの指定。**使用中のラベルを消すときは何行から外れるかを出してから消す**（Ctrl+Z で戻せる）。
- **複数行を選んで右クリック → Status / Tags で一括設定**できる。セルのプルダウンも、その行が選択に含まれていれば選択全体に効く。1 行だけ直したいのに黙って 1 行しか変わらない、の逆で、選択したものが全部変わる。
- **デフォルトステータスは、リストに入ってくる行に自動で付く**（フォルダ読み込み・D&D・ペースト）。セッションから復元される行は対象外で、**保存された割り当てを当てる前に全行のラベルを消してから**当てる。これをやらないと「明示的にステータスを外した行」が開き直すたびにデフォルトに戻ってしまう。
- **定義はアプリ設定とセッションの両方に持つ**。共有ドライブ上の `.nwsess` を他人が開いてもラベルと色がそのまま出るように、セッションが自分のパレットを抱えている。アプリ設定側は新規作業の出発点で、管理ウィンドウの「Save as global default」「Load global default」で行き来する。
- **ID はラベルから作った slug で、連番ではない**。共有セッションには書き手が複数いるので、連番だと 2 人が同時にラベルを足したときに同じ番号を取り合う。slug ならラベルが衝突したときだけ衝突し、そのときもラベル由来で解決する。ラベルのリネームや色変更では ID が動かないので、**行の割り当ては一切壊れない**。
- **どのパレットにも無い ID は、灰色バッジに生 ID を出してそのまま保持する**。他人のパレットで作られたセッションを開いて保存し直したときに、黙って捨てないため。セッションを読むだけで書き換えることはしない。
- 行は ID を `Arc<str>` でパレットと共有する（`display_folder` と同じ intern 方針）。Tags は Box に入れてあり、100 万行のリストでタグ無しの行に 24 バイト/行を払わない。

### 自分の実装を読み直して見つけた不具合を、出す前に直した

- **【重大】ステータスが無い古いセッションを開くと、ユーザーのパレットが消えていた。** セッション側のパレットを無条件で採用していたので、この機能より前に保存された `.nwsess`（= パレットのブロックが無い）を開くと、**空のパレットで上書き**されていた。その状態で prefs が保存されれば設定ごと消える。パレットのブロックが**そもそも無い**セッションは「ステータスは存在しない」ではなく「何も言っていない」なので、今のパレットに触らないようにした。デフォルトステータスも同じ。回帰テストを先に書いて、修正前に落ちることを確認した。
- **セッションを開いている間のパレット編集が、グローバル設定を書き換えていた。** 共有セッションを開いて 1 つラベルをリネームしただけで、自分のグローバルパレットが相手のもので置き換わる。セッションが開いている間、パレットはそのセッションのものなので、prefs への書き出しは**セッションが開いていないときだけ**に限定した（List Columns のウィンドウ位置がすでに同じ規則）。意図してグローバルへ移すのは管理ウィンドウの「Save as global default」。
- **表示中の全行ぶんの無駄な確保を毎フレームやっていた。** Status / Tags のセルが、開いてもいないプルダウンのためにパレットを clone し、さらに `selected_paths()`（= 選択全体の clone、大きなリストでは無制限）を毎フレーム呼んでいた。どちらもメニューが実際に開いたとき／実際に選ばれたときだけに遅らせた。
- **管理ウィンドウの使用行数を、定義ごとにリスト全走査で数えていた。** 定義 5 個 × 10 万行を毎フレーム。1 パスでまとめて数えるようにした。
- Status のバッジが列幅いっぱいに広がって、内容幅で並ぶ Tags のバッジと別物に見えていたので、内容幅・左寄せに揃えた。

### ステータスだけを変えた編集が Undo に積まれない不具合を直した

- リストの Undo 記録は「6 つのフィールドのどれかが動いたか」で判定していたが、その比較タプルが**2 つの関数に丸ごと複製**されていた。片方に新しいフィールドを足し忘れれば、その編集は「変化なし」と見なされて黙って Undo できなくなる。実際 Note の編集は今もそこに載っていない。
- 判定を `ListUndoFingerprint` 1 つに切り出して両方から呼ぶようにし、そこに status / tags を入れた。複製が消えたので、次にフィールドが増えたときに片方だけ直す余地が無くなった。
