...
- ↑/↓：選択移動、Enter：タブを開く。←/→：Gain(dB) を調整（既定 ±0.1 / Shift ±1.0 / Ctrl ±3.0）。
...
- ループ：LoopMode (Off/OnWhole/Marker) をトグル。Marker 時は Inspector > LoopEdit で設定した開始/終了とクロスフェード（長さ・シェイプ）を使用し、K/P でプレイヘッドから更新。Pitch/Stretch の処理後は出力レイテンシ/flush を考慮して末尾欠けを防止し、継ぎ目の引っかかりを低減。
...
- dBFS/LUFS 列は `-80..+6 dBFS`（および相当する LUFS）を黒→青→シアン→赤のトーンで補間。
