...
### リストの Comments 列（`src/app/ui/list/comment_cell.rs`）

会話を **ファイルの隣で気付く** ための面。ウィンドウはまとめて読む場所、列は
「この行について何か言われているか」を一覧で見て、その場で書く場所。

- セルは `💬 N`。**N はそのファイルを指すスレッド全体の件数**（返信を含む）。
  未読があれば青丸、根が全部 resolve 済みなら弱色。何も無い行は
  ホバーしたときだけ薄い `💬+` を出す（10 万行に 10 万個の吹き出しを描かない）。
- クリックでその行のスレッドをポップアップに開く。描画は**ウィンドウと同じ
  スレッド描画器**（`ui_comment_node`）で、Reply / Edit / Delete / Resolve も
  そのまま効く。2 つ目の簡易描画器を作らないのは、1 リリースで必ずずれるから。
- ポップアップのコンポーザーは、**新規スレッドなら本文末尾にそのファイルの
  参照トークンを自動で付ける**。「本文中のトークンが唯一の真実」なので、行から
  書いたのにどのファイルの話か記録されない、が起きないようにする。返信には
  付けない（スレッドの根が既に言っている）。
- **開いた時点でその行のスレッドは既読になる**（ウィンドウを開くのと同じ意味）。
- `⧉` でウィンドウを開く。その行を選択してから `This file` に切り替える
  （フィルタは選択に追従する設計なので、行を指すとは行を選ぶこと）。
- 列が非表示でも、行の右クリックメニューの `Comments...` から同じ場所へ行ける。
- ヘッダの `💬` はコメント件数でのソート。降順が先で、1 クリックで「何か
  言われている行」が上に来る。

件数は行ごとに数えない。`comment_ops::refresh_comment_path_index` が**会話が
動いたときだけ**スレッドを 1 周して `パス → (件数, 未解決, 未読)` を作り、
セルはハッシュ 1 回で読む。行ごとに本文を走査すると「行数 × コメント数」を
毎フレーム払うことになる。
...
- **`.nwsess` を保存する前のコメントは共有されない。** メモリに留まり、最初の
  Save で出る。UI に明示している。
- **列の件数はスレッド単位で、ファイル単位ではない。** 1 つのスレッドが 2 つの
  ファイルを指していれば、そのスレッドの件数は両方の行に出る。「このスレッドは
  A と B の話」が正しい読み方だが、合計しても総件数にはならない。
...
| `src/app/ui/comments.rs` | ウィンドウ、切り出し、スレッド描画、コンポーザー、`@` ピッカー、行ポップアップ |
| `src/app/ui/list/comment_cell.rs` | リストの Comments 列（バッジとポップアップの起動） |
...
| `tests/comments_ui.rs` | ウィンドウと Comments 列の操作 |
