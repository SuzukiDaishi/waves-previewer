...
- **ジョブ自体が失敗したら 5 / 15 / 60 / 300 秒とバックオフする。** ジョブ内の再試行は
  「誰かが先にコミットした」への答えでミリ秒で片付くが、共有先が落ちている場合は
  何度やっても失敗する。バックオフが無いと、前のワーカーが諦めた瞬間に次を起動して
  死んだパスを叩き続け、トーストも 0.5 秒ごとに出る。トーストは障害ごとに 1 回だけで、
  継続的な状態はウィンドウの「まだ共有されていません N 件」が担う。
