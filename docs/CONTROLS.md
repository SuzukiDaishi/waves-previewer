...
- `Tab` / `Shift+Tab`: 次 / 前の Editor タブへ（末尾で先頭へ回ります）。Editor ワークスペースが前面でタブが 2 つ以上あるときのみ。テキスト入力中・ダイアログ表示中・キー再割り当て中は egui のフォーカス移動のままです
- `Space`: 再生/停止
- `A` / `D`: 音量 Down / Up
- Volume コントロールを**ダブルクリック**すると 0.0 dB に戻ります（ラベル・トラック・数値表示のどこでも可）。最後に確定したモニター音量は prefs に保存され、次回起動時に復元されます（初回の既定値は 0.0 dB）
