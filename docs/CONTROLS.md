...
- `Space`: 再生/停止
- `A` / `D`: 音量 Down / Up
- Volume コントロールを**ダブルクリック**すると 0.0 dB に戻ります（ラベル・トラック・数値表示のどこでも可）。起動時の既定値も 0.0 dB です
...
- **ループ端（Loop Edit ツール）**: 縦線の上端に▼のハンドルが出ます（タイムストレッチのグリップのすぐ下）。
  ...
  - `R` の Zero Cross Snap が ON のときは、近くにマーカーが無ければゼロクロスに吸着します（優先順位は Marker > Zero Crossing）
