...
- `Tab` / `Shift+Tab`: 次 / 前のタブへ（末尾で先頭へ回ります）。**List も並びに含まれます** — `Ctrl+1..` と同じ順で `List → Editor タブ 1 → Editor タブ 2 → … → List`。Editor タブが 1 つも無いとき、および Recording / Effect Graph ワークスペースが前面のときは egui のフォーカス移動のままです（Recording / Effect Graph はこの循環には入りません）。テキスト入力中・ダイアログ表示中・キー再割り当て中も同様
- `Space`: 再生/停止
- `A` / `D`: 音量 Down / Up
- Volume コントロールを**ダブルクリック**するたびに `0 → -6 → -18 → -24 → 0 dB` と送られます（ラベル・トラック・数値表示のどこでも可）。最後に確定したモニター音量は prefs に保存され、次回起動時に復元されます（初回の既定値は 0.0 dB）
- Volume スライダーにフォーカスがあるとき、反応するのは `A` / `D` と左右キーだけです。`Space` は再生/停止に専念し、音量は動きません（egui はフォーカス中のウィジェットで Space をクリック扱いにするため、以前はマウスのある位置に音量が飛んでいました）。逆に左右キーはこのスライダーが占有し、Editor のシークやリストの Gain 調整には流れません。上下キーは List にフォーカスを戻して選択行を移動、`Enter` / `Esc` はフォーカスを外します
- **Volume のトラックは dB に対して等間隔ではありません**（無段階なのは従来どおり）。実際に調整する範囲である **0 dB 〜 -24 dB がトラックの約 45%** を占め、そこから下（-80 dB まで）は圧縮されています。dB 等間隔だと 0〜-24 dB が 1/4 強しか無く、少し動かすだけで大きく変わってしまうためです。0 dB（ユニティ）の位置には目盛りが入ります。左右キーは従来どおり 1 dB 刻み
...
## List View
- `Ctrl+C` / `Ctrl+V`: 選択ファイルをコピー / リストへ貼り付け。エクスプローラや Finder でコピーした音声ファイルも貼り付けられます（OS のファイルリスト、`file://` URI、改行区切りのパス、単一パスに対応）。追加件数と、重複 / 非対応 / 見つからないファイルの内訳をトーストで表示します
- `P`: Auto Play 切り替え
- `R`: Regex 検索切り替え
...
- **Status / Tags カラム**: 行に作業ステータス（1 行 1 つ）とタグ（1 行に複数）を付けられます。セルをクリックするとプルダウンが出ます（Tags はチェック式で、続けて複数選べるよう開いたままになります）。**その行が選択に含まれている場合は選択全体に適用**されます。複数行を選んで右クリック → **Status / Tags** でも一括設定できます。Status は既定で表示、Tags は既定で非表示です。
- **ステータス / タグの定義**: **Tools > Statuses & Tags...** で追加・リネーム・色・並び順・削除と、**デフォルトステータス**（リストに新しく入ってくる行へ自動で付く）を設定します。使用中のラベルを削除するときは何行から外れるかを確認してから消し、Ctrl+Z で戻せます。定義はアプリ設定とセッション(.nwsess)の両方に保存されるので、**共有した .nwsess を別の PC で開いてもラベルと色がそのまま**出ます。両者の行き来は管理ウィンドウの「Save as global default」「Load global default」です。
...
- **QA カラム**: Settings の List Columns で **Edge0 / Over0 / Blank** を表示できます。いずれもフルデコード時に算出し、**問題のある行だけ赤背景で `NG`** を表示（正常は空欄、未解析は `...`）。`NG` にマウスを乗せると理由と実測値が出ます。ソートすると `NG` が上に来ます。
  - **Edge0**: 先頭/末尾のサンプルが 0 でない（全チャンネルの絶対値が Settings の zero-cross epsilon を超える）。しきい値変更は再デコードなしで即反映されます。
  - **Over0**: サンプルピークが 0 dBFS を超える。Peak 列と揃えるため pending gain を含めて判定します。
  - **Blank**: 先頭または末尾のブランクが規定量に達している。Settings > **Blank Pad column** の **Threshold**（既定 -45.0 dBFS）と **Min length**（既定 10 ms）で調整します。Min length の変更は再デコード不要、Threshold の変更は表示中の行から順に測り直します。
- **Length カラムの時間表記**: 読み込んだリストに 1 時間以上のファイルが 1 つでもあると、全行が `h:mm:ss` 表記に切り替わります（従来は総分数表記で 2 時間が `120:11` になっていました）。CSV エクスポートも同じ規則に従います。
...
- `K`: ループ開始位置を現在再生位置で設定
- `P`: ループ終了位置を現在再生位置で設定
- `L`: ループ領域があれば Marker loop を有効化 / 無ければ選択範囲からループを作成 / どちらも無ければループモードを切り替え
- `Shift+L`: ループモードを切り替え（Off → 全体 → Marker → Off）。ループ領域があるときは Marker に固定されます
- `Ctrl+A`: ファイル全体を選択（インスペクタの範囲表示が `(entire file)` になり、トーストでも通知します）
- `Shift+Z`: ループ領域へズーム（`Z` の選択範囲版）
- `S`: 表示モード切り替え（Waveform -> Spectrogram -> Freq Log -> Mel -> Tempogram -> Chromagram -> World (F0/Env)）
- `B`: BPM 有効/無効
- `M`: 再生位置にマーカー追加
- `T`: 選択範囲を Trim 適用（実行後に Ctrl+Z 案内トーストを表示）
- 選択範囲の端の縦線をドラッグ（ツール非依存）: 選択範囲を伸縮して微調整。**範囲が変わるだけで音声は変わりません**。掴んだ側だけが動き、反対の端は固定されます（`Alt` でゼロクロススナップ、再生位置の近くでは再生位置にスナップ）。反対の端を追い越すと、範囲を引き直したときと同じように前後が入れ替わります
- 選択範囲の端の**上端のグリップ**（キャンバス最上部の水色のつまみ、Waveform ビューのみ）をドラッグ: その範囲をタイムストレッチ（0.25x〜4x、ピッチ保持）。こちらは**音声そのものを書き換えます**。ホバーすると手のひらカーソルになり、縦線の他の部分（↔ カーソル）と区別できます。破壊的なほうを小さな的に、無害なほうを大きな的に割り当てているためです
  - Loop Edit ツールでループ端と選択端が重なっているときも同じ分担です。上端のグリップはストレッチ、その下の縦線はループマーカーの移動になります
- **ループ端（Loop Edit ツール）**: 縦線の上端に▼のハンドルが出ます（タイムストレッチのグリップのすぐ下）。
  - **クリック = 再生位置の移動**、**ドラッグ = ループ点の移動**です。以前はクリックした瞬間にループ点が動いていました
  - ドラッグ中はマーカーに吸着します（画面上 8px 以内）。吸着したループ点はマーカーとまったく同じ sample になります
  - 通常ドラッグは任意のサンプル位置へ移動し、`Alt` を押している間だけ、近くにマーカーが無ければゼロクロスへ吸着します（優先順位は Marker > Zero Crossing）
  - ハンドルを**ダブルクリック**すると、その端を中心に 1 段ズームインします
- **Spectrogram / Freq Log / Mel の色レンジ**: キャンバス右側のdBレールで上下のハンドルをドラッグします。ダブルクリックで -120～0 dBへ戻り、選んだハンドルは上下キーで1 dBずつ調整できます。色レンジ変更は解析をやり直さず、表示だけを更新します
- `Ctrl+M`: 選択範囲をミュート（無音化。実行後に Ctrl+Z 案内トーストを表示）
- `Delete`: 選択範囲を削除して詰める（Trim インスペクタの Mode=Cut と同じ。実行後に Ctrl+Z 案内トーストを表示）
...
- `Left` / `Right`: 時間 / BPM グリッド刻みでシーク。**マーカーとループ端を跨ぐときはそこで止まります**（`Shift` で選択範囲を拡張、`Alt` でゼロクロス、`Ctrl` で 1 サンプル）
...
- **Shift+ホイールの横スクロール**は「表示幅の 15%／1 ノッチ」になりました（従来は固定 60px）。長尺でもズーム率に関係なく同じ感覚で流れます。Settings の **Horizontal scroll speed**（1x / 2x / 4x、既定 1x）で倍率を上げられます。
  - Ctrl+Shift+ホイールは使えません。egui が COMMAND をズーム修飾キーとして先に横取りするため、Ctrl+ホイール と区別できないためです。
...
- **Channel Routing** ツール: チャンネルの入れ替え・複製・削除を行うパッチベイです。左が元ファイルのチャンネル、右が出力チャンネル。出力チャンネル数は 1〜8 で変更でき、`Swap L/R` / `Mono → Stereo` / `→ Mono` / `Identity` のワンクリックプリセットもあります。
  - **繋ぐ**: 入力ピンから出力ピンへ**ドラッグ**、または入力ピンをクリック → 出力ピンをクリック。
  - **切る**: **ケーブルを直接クリック**します（カーソルを近づけると赤くハイライトされ ✂ が出ます）。ピンを**右クリック**するとそのピンのケーブルを一括で外せます。
  - 当たり判定は見た目より大きく取ってあり、ピンは半径 18px、ケーブルは 11px です（ピンが優先されるので、ケーブルの端がピンを塞ぐことはありません）。カーソル下に薄い円で有効範囲が出ます。
  - 1 つの出力に複数の入力を繋ぐと**平均**されます（L+R→mono で 0 dBFS を超えません）。ケーブルの無い出力は無音になります。
  - ファイル全体が対象で、選択範囲とチャンネルビューは無視されます。Apply は Undo 履歴に入ります。
  - エディタで唯一チャンネル数が変わるツールのため、適用時に mute/solo とチャンネルビューはリセットされます。
...
- Speed / Time Stretch ツール: 範囲選択後、選択範囲の端にある上端のグリップを左右にドラッグして伸縮（0.25x〜4x）。このグリップはツール非依存で、どのツールを選んでいても使えます。ドラッグ中はゴースト領域とレート表示、離すと処理後の波形をプレビュー。選択範囲のみが処理され、境界はクロスフェードで滑らかに接続。
...
## Debug Window のフレーム計測

- `F12` または Tools → Debug Window で **Frame Profiler** を開けます。
- **Actual FPS** は実際のアプリフレーム間隔、**UI Thread Time** はCPU側の更新・描画準備を区間別に積み上げた値です。
- **Top UI-thread blockers** は直近履歴をP95順に並べます。赤は単独で16.67msを超える処理、黄は4ms以上を使う処理です。
- **Hold** でグラフを固定し、**Clear** で最大600フレームの履歴を消去できます。計測はDebug Window表示中だけ動作します。
- GPUのPresent時間はeguiから取得できないため、GPU負荷は「FPSが低いのにUI Thread Timeが短い」場合の候補として判断します。

## Notes
- `S` は Editor では View 切り替え専用です。ゼロクロス吸着はループ端・選択端を動かす間の `Alt` 修飾です。
- List と Editor で同じキーでも意味が異なるものがあります（例: `P`）。
