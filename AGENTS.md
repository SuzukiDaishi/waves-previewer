...
- `docs/`: Design/refactor plans and specs.
  ...
  - `COMMENTS_SPEC.md`: the shared session's conversation -- schema, the write path that avoids the conflict prompt, and how a colleague's comment is told apart from a colleague's save.
...
- `src/`: Rust sources (app + engine).
  - `src/app/`: WavesPreviewer implementation split by feature.
    ...
    - `session_watch.rs`: polling probe that notices when somebody else saves the open session. Reports only -- reloading is the user's decision, because an automatic reload discards unsaved edits. It now holds that report until a comment pull says whether the document changed in a way worth warning about: posting rewrites the shared file, so without that every colleague's comment would stand up a warning whose remedy throws away edits.
    - `comments.rs`: the rules a shared session's conversation obeys, with no UI and no disk -- random ids (never a counter, which two writers collide on), the id-keyed set union that makes two people's copies mergeable, the reply-forest builder that promotes an orphan or a cycle to a root rather than dropping somebody's words, and the `@[path|start-end|low-highHz]` reference grammar. Times here are seconds against the source file, not sample indices like `EditorNote`: a comment points at the file a colleague will open, not into one person's editor buffer.
    - `comment_ops.rs`: posting, editing, withdrawing and resolving, plus the write job. Also the per-file view of the conversation the list's Comments column reads (`refresh_comment_path_index`) -- rebuilt when the conversation moves, never per row, because the alternative is scanning every body for reference tokens once per visible row per frame. A comment never goes out through the session save -- that would push the author's unsaved edits with it and raise the conflict prompt whenever two people typed at once. It reads the document *on disk*, unions the outbox into its conversation and compare-and-swaps it back; a CAS miss is not a conflict but a reason to re-read and re-merge, so it converges without a prompt. Also the two-phase reference jump, modelled on `transcript_ops.rs`.
...
- A `.nwsess` may have more than one writer (two GUI instances, or a GUI and a `--cli` batch, against a file server). There is deliberately no lock. Any new session write goes through the compare-and-swap in `run_session_save_jobs` / `cli_ops::write_project_file_checked` -- never a bare `fs::write` -- and any new file the session owns is named after its contents, never after an index or a counter, because both of those are shared between writers and collide. The same goes for any *id* stored in the document: `comments.rs` issues random ones for the same reason `status_tags.rs` uses slugs.
- Anything added to the document that several people write concurrently needs a merge rule, or it needs to stay out. Comments have one (a set union keyed by id, settled by `(rev, edited_at)`), which is what lets `comment_ops` retry a lost compare-and-swap instead of prompting. Anything without such a rule falls back to the whole-document choice the conflict modal offers.
