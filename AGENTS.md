...
- `src/`: Rust sources (app + engine).
  - `src/app/`: WavesPreviewer implementation split by feature.
    ...
    - `comment_ops.rs`: posting, editing, withdrawing and resolving, plus the write job. A comment never goes out through the session save -- that would push the author's unsaved edits with it and raise the conflict prompt whenever two people typed at once. It reads the document *on disk*, unions the outbox into its conversation and compare-and-swaps it back; a CAS miss is not a conflict but a reason to re-read and re-merge, so it converges without a prompt. Also the two-phase reference jump, modelled on `transcript_ops.rs`.
