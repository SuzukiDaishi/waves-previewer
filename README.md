...
- スクリプト本体は `installer\\NeoWaves.nsi` です。`makensis` は POSIX ホストでも動くので、Windows 以外からでも `makensis -DAPP_VERSION=... -DSRC_DIR=... -DOUT_FILE=... installer/NeoWaves.nsi` でスクリプト自体の検証ができます。
