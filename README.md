...
- ステータス列（1 行 1 つ）とタグ列（1 行に複数）。行のプルダウンから、または
  複数行を選んで右クリック → Status / Tags で一括設定。ラベルの追加・リネーム・
  色・並び順・デフォルトステータスは Tools → Statuses & Tags…
  定義はアプリ設定とセッション(.nwsess)の両方に保存されるので、共有した
  セッションを他の PC で開いてもラベルと色がそのまま出ます
...
### コメント（複数人で 1 つのセッションを読み合う）
共有フォルダの `.nwsess` に、**誰がどの音のどこに何を言ったか**を残せます。
`File > Comments...`（Ctrl+Shift+M）。

- **スレッド。** 特定のコメントに返信でき、片付いた話は Resolve で畳めます。
- **参照。** コメント本文にファイル・時刻・時間範囲・周波数帯を埋め込めます。
  押すとその位置に飛び、範囲ならエディタの選択まで復元されます。入れ方は
  `🔗 Reference` メニュー（今のファイル / 再生位置 / 選択範囲）、`@` で始めて
  ファイル名を打つ、**Alt を押しながらリスト行をドラッグ**、ウィンドウへの
  ファイルドロップ、の 4 通り。素のドラッグは従来どおり DAW への配置です。
- **書式。** `**太字**` `*斜体*` `` `code` `` `- 箇条書き` `> 引用` など最小限の
  Markdown が使えます。
- **別ウィンドウ。** `⧉` でコメントだけを独立した OS ウィンドウに切り出せます。
  別モニタに置いたまま波形を操作できます。
- **未読。** 他の人の新しいコメントはトップバーに `💬 N new` と出ます。既読は
  自分の PC にだけ記録されるので、共有ファイルは汚れません。
- ID は **PC のユーザ名**（表示名は prefs.txt の `display_name=`）。同じユーザ名の
  人が複数いるときだけマシン名が併記されます。

**保存で困りません。** コメントの投稿はセッション全体の保存とは別扱いで、

- あなたの未保存の編集を一緒に書き出しません。
- 2 人が同時に書いても競合ダイアログは出ません（自動でマージして書き直します）。
- 同僚がコメントしただけなら `⟳ changed on disk` は出ませんし、あなたの Ctrl+S も
  邪魔されません。
- まだ保存していないセッションのコメントは、最初の保存で一緒に出ます。

詳細は `docs/COMMENTS_SPEC.md`。
